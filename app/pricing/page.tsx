...
export default function PricingPage() {
    ...
    return (
        <>
            <Navbar />
            <div className="min-h-screen bg-black text-white flex flex-col items-center py-20 px-4 md:px-8 pt-32">
                ...
                {/* Toggle */}
                <div className="flex bg-[#111] border border-[#222] rounded-full p-1 mb-16">
                    <button
                        onClick={() => setBillingCycle('monthly')}
                        className={`px-6 py-2.5 rounded-full text-sm font-medium transition-colors ${billingCycle === 'monthly' ? 'bg-[#222] text-white' : 'text-gray-400 hover:text-white'}`}
                    >
                        Sapi Perah (Dairy)
                    </button>
                    <button
                        onClick={() => setBillingCycle('yearly')}
                        className={`px-6 py-2.5 rounded-full text-sm font-medium transition-colors ${billingCycle === 'yearly' ? 'bg-[#222] text-white' : 'text-gray-400 hover:text-white'}`}
                    >
                        Sapi Potong (Beef)
                    </button>
                </div>
                ...
                {/* Functional Slider */}
                <div className="w-full max-w-4xl mb-16 relative px-4 hidden md:block">
                    <div className="relative w-full flex flex-col items-center">

                        {/* Label jumlah sapi yang dipilih di atas slider */}
                        <div className="mb-6 bg-[#222] px-4 py-1.5 rounded-full text-sm font-medium border border-gray-700 text-white shadow-lg">
                        ...
                        <input
                            ...
                            className="w-full h-1.5 bg-gray-800 rounded-lg appearance-none cursor-pointer accent-white focus:outline-none"
                            style={{
                                background: `linear-gradient(to right, white ${(sliderIndex / (sliderMarks.length - 1)) * 100}%, #1f2937 ${(sliderIndex / (sliderMarks.length - 1)) * 100}%)`
                            }}
                        />
                        ...
                        <div className="flex justify-between w-full mt-4 text-[12px] text-gray-500 font-medium">
                            {sliderMarks.map((mark, i) => (
                                <span
                                    key={i}
                                    className={`cursor-pointer transition-colors ${sliderIndex === i ? 'text-white' : 'hover:text-gray-300'}`}
                ...
                <div className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {pricingTiers.map((tier, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className={`bg-black border transition-all duration-300 rounded-[24px] p-8 flex flex-col relative
                  ${isActive ? 'border-emerald-500/50 shadow-[0_0_30px_rgba(16,185,129,0.1)] scale-[1.02]' : 'border-gray-800 hover:border-gray-700'}
                  ${isDimmed ? 'opacity-40 grayscale-[50%]' : 'opacity-100'}
                `}
                            >

                                {isActive && (
                                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-3 py-1 rounded-full text-xs font-medium">
                                        Rekomendasi
                                    </div>
                                )}

                                <h3 className={`text-center text-sm font-medium mb-6 ${isActive ? 'text-emerald-400' : 'text-white'}`}>
                                    {tier.title}
                                </h3>
                                ...
                                <div className="text-center mb-6">
                                    <p className="text-sm text-white font-medium mb-1.5">{tier.mainLimit}</p>
                                ...
                                <ul className="flex flex-col gap-4 flex-grow">
                                    {tier.features.map((feature, fIndex) => (
                                        <li key={fIndex} className="flex items-start">
                                            ...
                                            <span className={`text-[13px] leading-relaxed ${feature.included ? 'text-gray-300' : 'text-gray-600'}`}>
                                                {feature.name}
                                            </span>
                                        </li>
                                    ))}
                ...
                <div className="w-full max-w-5xl mt-32 px-4">
                    <h2 className="text-3xl font-medium text-white mb-8 tracking-tight">Manajemen & Pencatatan</h2>

                    <div className="w-full overflow-x-auto pb-4">
                        <div className="min-w-[768px]">
                            {/* Table Header */}
                            <div className="grid grid-cols-5 gap-4 bg-[#111] border border-gray-800 rounded-lg py-3 px-6 mb-2">
                                <div className="col-span-1"></div>
                                <div className="text-center text-[13px] font-medium text-gray-400">Skala Kecil</div>
                                <div className="text-center text-[13px] font-medium text-gray-400">Skala Menengah</div>
                                <div className="text-center text-[13px] font-medium text-gray-400">Skala Besar</div>
                                <div className="text-center text-[13px] font-medium text-gray-400">Korporasi</div>
                            </div>
                            ...
                            <div className="flex flex-col">
                                {[
                                    ...
                                    <div key={rowIndex} className="grid grid-cols-5 gap-4 py-4 px-6 border-b border-gray-800/60 hover:bg-[#0a0a0a] transition-colors">
                                        <div className="col-span-1 flex items-center">
                                            <span className="text-[13px] text-gray-300 border-b border-dotted border-gray-600 pb-0.5 cursor-help">
                                                {row.name}
                                            </span>
                                        </div>
                                        {row.values.map((val, valIndex) => (
                                            <div key={valIndex} className="flex justify-center items-center text-[13px] text-gray-300">
                                                {typeof val === 'boolean' ? (
                                                    val ? <CheckIcon /> : <span className="text-gray-700">-</span>
                                                ) : (
                                                    val as string
                                                )}
                                            </div>
                ...
                {/* FAQ Section */}
                <div className="w-full max-w-4xl mt-32 px-4 mb-20">
                    <h2 className="text-3xl font-medium text-white mb-8 tracking-tight">Frequently Asked Questions</h2>
                    ...
                    <div className="flex flex-col gap-3">
                        {faqs.map((faq, index) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className="bg-[#151515] border border-gray-800/60 rounded-xl overflow-hidden"
                                >
                                    <button
                                        onClick={() => setOpenFaqIndex(isOpen ? -1 : index)}
                                        className="w-full flex items-center gap-4 px-6 py-5 text-left hover:bg-[#1a1a1a] transition-colors focus:outline-none"
                                    >
                                        <ChevronIcon isOpen={isOpen} />
                                        <span className="text-[15px] font-medium text-gray-200">{faq.q}</span>
                                    </button>
