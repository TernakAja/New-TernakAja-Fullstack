...
// --- Static icons (outside component, never recreated) ---
const CheckIcon = () => (
...
// --- Static data (outside component, allocated once) ---
...
const formatRp = (num: number) => rpFormatter.format(num);

const SLIDER_MARKS = [10, 30, 50, 100, 200, 500, 1000];

// Tier definitions with static fields extracted. Only Skala Besar's
// calculatePrice actually depends on runtime data; the others return constants.
const PRICING_TIERS = [
    {
        title: "Skala Kecil",
        basePrice: "Rp80.000",
...
// Comparison table rows — static, defined once
...
// Pure helper — stable reference, no closure over state
function getActiveTierIndex(cows: number): number {
    if (cows < 30) return 0;
...
export default function PricingPage() {
    ...
    // Only recomputed when sliderIndex changes
    const activeTierIndex = useMemo(() => getActiveTierIndex(selectedCows), [selectedCows]);

    // Slider fill style — only recomputed when sliderIndex changes
